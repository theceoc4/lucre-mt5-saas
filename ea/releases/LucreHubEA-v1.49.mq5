//+------------------------------------------------------------------+
//|                                                  LucreHubEA.mq5   |
//|  v1.0.49 — Lucre Hub main Expert Advisor (single-file build)      |
//|                                                                    |
...
datetime g_ews_last_position_state_sent_at = 0;
...
string EAStream_BuildPositionState()
{
   ...
   return "{\"instance_id\":\"" + EASync_JsonEscape(EASync_GetInstanceId()) + "\","
      "\"account_floating_pl\":" + DoubleToString(AccountInfoDouble(ACCOUNT_PROFIT), 2) + ","
...
void EAStream_BroadcastPositionState(const bool force)
{
   ...
   if(!force && state_json == g_ews_last_position_state &&
      now - g_ews_last_position_state_sent_at < EWS_POSITION_HEARTBEAT_SECONDS) return;
   ...
   if(status == 200)
   {
      ...
      g_ews_last_position_state_sent_at = now;
      return;
   }
...
void EAStream_HandleFrame(int opcode, const uchar &payload[], int payload_len)
{
   if(opcode == 0x1) // text
   {
         ...
         StringFind(text, "private_position_lease") >= 0)
      {
         ...
         g_ews_position_stream_requested_until = TimeLocal() + 45;
         if(stream_was_inactive)
         {
            g_ews_last_position_state = "";
            g_ews_last_position_state_sent_at = 0;
