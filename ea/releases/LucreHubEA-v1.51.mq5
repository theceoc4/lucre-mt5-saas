//+------------------------------------------------------------------+
//|                                                  LucreHubEA.mq5   |
//|  v1.0.51 — Lucre Hub main Expert Advisor (single-file build)      |
//|                                                                    |
...
//|      (SymbolMap module)                                              |
//|    - reports broker-native closed bars for every enabled timeframe   |
//|      so backend indicators and strategies share one durable feed     |
//|      (PriceReporter module)                                          |
//|    - reads configured custom-indicator buffers on closed candles and  |
//|      relays candidates into the same server-side strategy engine      |
//|      (IndicatorBridge module)                                         |
//|                                                                     |
...
//|  include was missing. This build inlines all modules directly      |
...
ulong    g_es_last_poll_ms        = 0;
ulong    g_es_last_command_poll_ms = 0;
...
void EASync_Init(const string base_url, const string api_key, const int poll_seconds = 2)
{
   ...
   g_es_last_poll_ms = 0; // force an immediate first poll on the next OnTimer() tick
   g_es_last_command_poll_ms = 0;
   g_es_force_full_sync = false;
   g_es_command_lane_running = false;
   g_es_standby_until = 0;
...
bool EASync_IsStandby()
{
   return g_es_standby_until > TimeLocal();
}

void EASync_HandleStandbyResponse(const int status, const string response)
{
   if(status != 409 || StringFind(response, "ea_instance_standby") < 0) return;
   g_es_standby_until = TimeLocal() + 45;
   Print("EASync: another EA owns this terminal lease; pausing all network lanes for 45 seconds");
}
...
void EASync_OnTimer()
{
   ...
   if(!g_es_force_full_sync && g_es_last_poll_ms > 0 &&
      now_ms - g_es_last_poll_ms < (ulong)g_es_poll_seconds * 1000)
      return;
...
//+------------------------------------------------------------------+
void EASync_GetOpenPositionCosts(const ulong position_id, double &commission, double &fee)
{
   commission = 0.0;
   fee = 0.0;
   if(position_id == 0 || !HistorySelectByPosition(position_id)) return;

   const int deal_total = HistoryDealsTotal();
   for(int deal_i = 0; deal_i < deal_total; deal_i++)
   {
      const ulong deal_ticket = HistoryDealGetTicket(deal_i);
      if(deal_ticket == 0) continue;
      const long entry = HistoryDealGetInteger(deal_ticket, DEAL_ENTRY);
      // Only costs attached to the still-open exposure belong in live P/L.
      // Exit costs from a partial close are already realized in balance.
...
{
   double positions_profit = 0.0;
   double positions_swap = 0.0;
   double positions_commission = 0.0;
   double positions_fee = 0.0;
   int account_position_total = PositionsTotal();
   for(int account_i = 0; account_i < account_position_total; account_i++)
   {
      ulong account_ticket = PositionGetTicket(account_i);
      if(account_ticket == 0) continue;
      positions_profit += PositionGetDouble(POSITION_PROFIT);
      positions_swap += PositionGetDouble(POSITION_SWAP);
      double position_commission = 0.0;
      double position_fee = 0.0;
      EASync_GetOpenPositionCosts((ulong)PositionGetInteger(POSITION_IDENTIFIER), position_commission, position_fee);
      positions_commission += position_commission;
      positions_fee += position_fee;
   }
   const double net_floating_pl = positions_profit + positions_swap + positions_commission + positions_fee;
   ...
   string account_json =
      ...
      "\"floating_pl\":" + DoubleToString(net_floating_pl, 2) + ","
      "\"credit\":" + DoubleToString(AccountInfoDouble(ACCOUNT_CREDIT), 2) + ","
      "\"positions_profit\":" + DoubleToString(positions_profit, 2) + ","
      "\"positions_swap\":" + DoubleToString(positions_swap, 2) + ","
      "\"positions_commission\":" + DoubleToString(positions_commission, 2) + ","
      "\"positions_fee\":" + DoubleToString(positions_fee, 2) + ","
   ...
   int total = PositionsTotal();
   int appended_positions = 0;
   for(int i = 0; i < total; i++)
   {
      ulong ticket = PositionGetTicket(i);
      if(ticket == 0) continue;
      if(appended_positions > 0) positions_json += ",";
      appended_positions++;
      ...
      double fee = 0.0;
      EASync_GetOpenPositionCosts((ulong)PositionGetInteger(POSITION_IDENTIFIER), commission, fee);
...
bool EASync_CommandExchange()
{
   ...
   if(status != 200)
   {
      EASync_HandleStandbyResponse(status, response);
...
{
   if(g_es_base_url == "" || g_es_api_key == "" || g_es_command_lane_running || EASync_IsStandby())
      return;

   g_es_command_lane_running = true;
   g_es_last_command_poll_ms = GetTickCount64();
...
{
   if(EASync_IsStandby()) return;
   ulong now_ms = GetTickCount64();
   if(g_es_last_command_poll_ms > 0 && now_ms - g_es_last_command_poll_ms < 1000) return;
...
{
   if(EASync_IsStandby()) return;
   g_es_last_poll_ms = GetTickCount64();
...
const int EWS_POSITION_HEARTBEAT_SECONDS = 8;
ulong    g_ews_last_position_stream_check_ms = 0;
ulong    g_ews_position_stream_requested_until_ms = 0;
string   g_ews_last_position_state = "";
ulong    g_ews_last_position_state_sent_at_ms = 0;
ulong    g_ews_last_attempt_ms      = 0;
int      g_ews_backoff_seconds   = 2;
ulong    g_ews_last_activity_ms     = 0;
ulong    g_ews_last_ping_sent_ms    = 0;
ulong    g_ews_handshake_started_ms = 0;
...
string EAStream_BuildPositionState()
{
   ...
   for(int i = 0; i < total; i++)
   {
      ...
      double fee = 0.0;
      EASync_GetOpenPositionCosts((ulong)PositionGetInteger(POSITION_IDENTIFIER), commission, fee);
      positions_profit += profit;
      positions_swap += swap;
      positions_commission += commission;
      positions_fee += fee;
      positions +=
         "{\"mt5_ticket\":" + IntegerToString((long)ticket) + ","
   ...
   return "{\"instance_id\":\"" + EASync_JsonEscape(EASync_GetInstanceId()) + "\","
      "\"account_floating_pl\":" + DoubleToString(positions_profit + positions_swap + positions_commission + positions_fee, 2) + ","
      "\"account_credit\":" + DoubleToString(AccountInfoDouble(ACCOUNT_CREDIT), 2) + ","
      "\"positions_profit\":" + DoubleToString(positions_profit, 2) + ","
      "\"positions_swap\":" + DoubleToString(positions_swap, 2) + ","
      "\"positions_commission\":" + DoubleToString(positions_commission, 2) + ","
      "\"positions_fee\":" + DoubleToString(positions_fee, 2) + ","
...
{
   if(!g_ews_joined || EASync_IsStandby()) return;
   ulong now_ms = GetTickCount64();
   ...
   if(now_ms > g_ews_position_stream_requested_until_ms) return;
   if(!force && g_ews_last_position_stream_check_ms > 0 &&
      now_ms - g_ews_last_position_stream_check_ms < (ulong)g_ews_position_stream_seconds * 1000) return;
   g_ews_last_position_stream_check_ms = now_ms;

   string state_json = EAStream_BuildPositionState();
   if(!force && state_json == g_ews_last_position_state &&
      g_ews_last_position_state_sent_at_ms > 0 &&
      now_ms - g_ews_last_position_state_sent_at_ms < (ulong)EWS_POSITION_HEARTBEAT_SECONDS * 1000) return;
   ...
   if(status == 200)
   {
      ...
      g_ews_last_position_state_sent_at_ms = now_ms;
      return;
   }
   ...
   EASync_HandleStandbyResponse(status, detail);
...
void EAStream_HandleFrame(int opcode, const uchar &payload[], int payload_len)
{
   if(opcode == 0x1) // text
   {
      ...
      {
         // The elected dashboard tab renews every 25s. If every viewer leaves,
         // broadcasts stop automatically after this short grace period.
         ulong now_ms = GetTickCount64();
         bool stream_was_inactive = now_ms > g_ews_position_stream_requested_until_ms;
         g_ews_position_stream_requested_until_ms = now_ms + 45000;
         if(stream_was_inactive)
         {
            g_ews_last_position_state = "";
            g_ews_last_position_state_sent_at_ms = 0;
            g_ews_last_position_stream_check_ms = 0;
            Print("EAStream: private dashboard position stream leased");
...
void EAStream_PumpFrames()
{
   ...
   if(available > 0)
   {
      ...
      if(got > 0)
      {
         ...
         g_ews_last_activity_ms = GetTickCount64();
      }
   }
...
{
   g_ews_last_attempt_ms = GetTickCount64();
   ...
   g_ews_handshake_started_ms = GetTickCount64();
   g_ews_hs_len = 0;
...
{
   if(GetTickCount64() - g_ews_handshake_started_ms > (ulong)EWS_HANDSHAKE_TIMEOUT * 1000)
   {
   ...
   g_ews_backoff_seconds = 2;
   g_ews_last_activity_ms = GetTickCount64();
   g_ews_last_ping_sent_ms = GetTickCount64();
   EAStream_SendJoin();
                   ...
                   const int position_stream_seconds = 2)
{
   ...
   g_ews_join_ref = "";
   g_ews_last_position_stream_check_ms = 0;
   g_ews_position_stream_requested_until_ms = 0;
   g_ews_last_position_state = "";
   g_ews_last_position_state_sent_at_ms = 0;
   g_ews_last_attempt_ms = 0; // forces an immediate first connect attempt
   g_ews_backoff_seconds = 2;
...
void EAStream_OnTimer()
{
   ...
   {
      if(g_ews_last_attempt_ms == 0 ||
         GetTickCount64() - g_ews_last_attempt_ms >= (ulong)g_ews_backoff_seconds * 1000)
         EAStream_Connect();
      return;
   }
   ...
   if(g_ews_state == EWS_STATE_OPEN)
   {
      ...
      if(g_ews_last_activity_ms > 0 &&
         GetTickCount64() - g_ews_last_activity_ms > (ulong)EWS_STALE_SECONDS * 1000)
      {
      ...
      if(g_ews_last_ping_sent_ms == 0 ||
         GetTickCount64() - g_ews_last_ping_sent_ms >= (ulong)EWS_PING_INTERVAL * 1000)
      {
...
{
   for(int i = 0; i < ArraySize(g_pr_sync_keys); i++)
      if(g_pr_sync_keys[i] == series_key) return i;
   if(!create) return -1;
   int index = ArraySize(g_pr_sync_keys);
   ArrayResize(g_pr_sync_keys, index + 1);
   ArrayResize(g_pr_sync_retry_at, index + 1);
   ArrayResize(g_pr_sync_attempts, index + 1);
   g_pr_sync_keys[index] = series_key;
   g_pr_sync_retry_at[index] = 0;
   g_pr_sync_attempts[index] = 0;
   return index;
}

bool PriceReporter_SyncWaiting(const string series_key, const datetime now)
{
   int index = PriceReporter_SyncIndex(series_key, false);
   return index >= 0 && g_pr_sync_retry_at[index] > now;
}

int PriceReporter_SyncAttempt(const string series_key)
{
   int index = PriceReporter_SyncIndex(series_key, true);
   g_pr_sync_attempts[index]++;
   return g_pr_sync_attempts[index];
}

void PriceReporter_ScheduleSyncRetry(const string series_key, const datetime now, const int delay_seconds)
{
   int index = PriceReporter_SyncIndex(series_key, true);
   int safe_delay = (delay_seconds > 0) ? delay_seconds : 1;
   g_pr_sync_retry_at[index] = now + safe_delay;
}

int PriceReporter_SyncAttempts(const string series_key)
{
   int index = PriceReporter_SyncIndex(series_key, false);
   return index >= 0 ? g_pr_sync_attempts[index] : 0;
}

void PriceReporter_ClearSync(const string series_key)
{
   int index = PriceReporter_SyncIndex(series_key, false);
   if(index < 0) return;
   g_pr_sync_retry_at[index] = 0;
...
int PriceReporter_BootstrapIndex(const string series_key)
{
   for(int i = 0; i < ArraySize(g_pr_bootstrap_keys); i++)
      if(g_pr_bootstrap_keys[i] == series_key) return i;
   return -1;
}

int PriceReporter_CompletedBootstrapGeneration(const string series_key)
{
   int index = PriceReporter_BootstrapIndex(series_key);
   return (index >= 0) ? g_pr_bootstrap_generations[index] : 0;
}

void PriceReporter_MarkBootstrapComplete(const string series_key, const int generation)
{
   int index = PriceReporter_BootstrapIndex(series_key);
   if(index < 0)
   {
      index = ArraySize(g_pr_bootstrap_keys);
...
struct PriceReporterBoundSymbol
{
   ...
   string feed_bootstrap_generations;
   string feed_bootstrap_required;
   string feed_history_counts;
};
...
int PriceReporter_ServerBootstrapGeneration(const string generations_json, const string timeframe)
{
   if(generations_json == "") return 1;
   string raw = EASync_JsonGetRaw(generations_json, 0, StringLen(generations_json), timeframe);
   int generation = (int)StringToInteger(raw);
   return (generation > 0) ? generation : 1;
}

int PriceReporter_ServerHistoryCount(const string counts_json, const string timeframe)
{
   if(counts_json == "") return 0;
   string raw = EASync_JsonGetRaw(counts_json, 0, StringLen(counts_json), timeframe);
...
int PriceReporter_TimeframeSeconds(const string timeframe)
{
...
//| cannot be satisfied solely by an old local block whose positions  |
//| still exist. MT5 must construct/download data for the requested    |
//| dates, and a later timer event verifies the returned timestamps.   |
//+------------------------------------------------------------------+
int PriceReporter_CopyRecentWindow(const string broker_symbol,
                                   const ENUM_TIMEFRAMES period,
                                   const int timeframe_seconds,
                                   const datetime last_sent,
                                   const datetime now,
                                   MqlRates &rates[],
                                   datetime &current_bar_open)
{
   ArrayResize(rates, 0);
   current_bar_open = 0;
   datetime request_from = now - (timeframe_seconds * (PR_FRESHNESS_BARS + 2));
   if(last_sent > 0 && last_sent + 1 > request_from)
      request_from = last_sent + 1;
   if(request_from > now) request_from = now;

   ResetLastError();
   int copied = CopyRates(broker_symbol, period, request_from, now, rates);
   for(int i = 0; i < copied; i++)
      if(rates[i].time > current_bar_open) current_bar_open = rates[i].time;
   return copied;
}
   ...
   while(pos < json_len)
   {
      ...
      else if(ch == '}')
      {
         ...
         if(depth == 0 && obj_start >= 0)
         {
            ...
            string feed_bootstrap_generations = EASync_JsonGetRaw(json, obj_start, obj_end, "feed_bootstrap_generations");
            string feed_bootstrap_required = EASync_JsonGetRaw(json, obj_start, obj_end, "feed_bootstrap_required");
            string feed_history_counts = EASync_JsonGetRaw(json, obj_start, obj_end, "feed_history_counts");
            if(canonical != "" && broker != "")
            {
               ...
               out_symbols[count].feed_bootstrap_generations = (feed_bootstrap_generations == "") ? "{}" : feed_bootstrap_generations;
               out_symbols[count].feed_bootstrap_required = (feed_bootstrap_required == "") ? "[]" : feed_bootstrap_required;
               out_symbols[count].feed_history_counts = (feed_history_counts == "") ? "{}" : feed_history_counts;
               count++;
            }
            obj_start = -1;
         }
...
bool PriceReporter_SendPayload(const string symbols_json,
...
{
   datetime run_started = TimeCurrent();
   g_pr_last_report = run_started;
   bool freshness_due = (g_pr_last_freshness == 0 ||
                         run_started - g_pr_last_freshness >= PR_FRESHNESS_INTERVAL_SECONDS);
   bool diagnostics_due = (g_pr_last_diagnostics == 0 ||
                           run_started - g_pr_last_diagnostics >= PR_DIAGNOSTIC_INTERVAL_SECONDS);
   ...
   string candidate_required[];
   string candidate_generations_json[];
   string candidate_counts_json[];
   ...
   for(int pass = 0; pass < 2; pass++)
   {
      for(int i = 0; i < bound_count; i++)
      {
         for(int tf_index = 0; tf_index < ArraySize(timeframe_names); tf_index++)
         {
            ...
            if(StringFind(bound[i].report_timeframes, "\"" + timeframe + "\"") < 0) continue;
            bool is_priority = StringFind(bound[i].priority_timeframes, "\"" + timeframe + "\"") >= 0;
            if((pass == 0 && !is_priority) || (pass == 1 && is_priority)) continue;
            int index = ArraySize(candidate_brokers);
            ArrayResize(candidate_brokers, index + 1);
            ArrayResize(candidate_timeframes, index + 1);
            ArrayResize(candidate_checkpoints, index + 1);
            ArrayResize(candidate_required, index + 1);
            ArrayResize(candidate_generations_json, index + 1);
            ArrayResize(candidate_counts_json, index + 1);
            candidate_brokers[index] = bound[i].broker_symbol;
            candidate_timeframes[index] = timeframe;
            candidate_checkpoints[index] = bound[i].feed_checkpoints;
            candidate_required[index] = bound[i].feed_bootstrap_required;
            candidate_generations_json[index] = bound[i].feed_bootstrap_generations;
            candidate_counts_json[index] = bound[i].feed_history_counts;
   ...
   int generations[];
   ArrayResize(needs_bootstrap, candidate_count);
   ...
   for(int i = 0; i < candidate_count; i++)
   {
      ...
      // backoff instead of being converted into a full-history bootstrap.
      ...
      if(freshness_due && !base_bootstrap_needed &&
         !sync_active && !new_close_expected)
         continue;

      // Request the actual missing wall-clock window. The former positional
      // CopyRates(1, 3) call could be satisfied by any old local block because
      // positions 1..3 still existed. A date window containing the expected
      // new candle cannot report success until MT5 has built that window.
      MqlRates rates[];
      datetime actual_current_bar = 0;
      int copied = PriceReporter_CopyRecentWindow(
      ...
      if(copied <= 0)
      {
         ...
         // window can be empty. A current tick is the proof that MT5 actually
         ...
         needs_bootstrap[i] = base_bootstrap_needed;
         g_pr_live_sync_pending = true;
         PrintFormat("PriceReporter: exact-time window unavailable for %s %s, checkpoint=%s error=%d symbol_sync=%d series_sync=%d retry=%ds",
                     broker_symbol, timeframe, TimeToString(last_sent), copy_error,
                     (int)SymbolIsSynchronized(broker_symbol),
         ...
         if(diagnostics_due)
         {
            ...
            diagnostics_json +=
               "{\"broker_symbol\":\"" + EASync_JsonEscape(broker_symbol) + "\"," +
               "\"timeframe\":\"" + timeframe + "\",\"state\":\"waiting_history\"," +
               "\"last_error\":\"Expected candle after " + EASync_ToIso8601(last_sent) +
                  "; exact-time CopyRates error " + IntegerToString(copy_error) + "\"," +
               "\"attempt_count\":" + IntegerToString(attempt) + "," +
               "\"retry_after_seconds\":" + IntegerToString(retry_seconds) + "}";
            diagnostic_count++;
         }
         continue;
      }
      ...
      if(expected_candle_missing)
      {
         ...
         // series build, but the expected closed bar is not ready yet. Keep the
         // live lane pending and verify again on a later timer event.
         int sync_index = PriceReporter_SyncIndex(series_key, true);
         if(g_pr_sync_retry_at[sync_index] > run_started)
         {
            collector_ready[i] = false;
            needs_bootstrap[i] = base_bootstrap_needed;
            g_pr_live_sync_pending = true;
            if(diagnostics_due)
            {
               ...
               diagnostics_json +=
                  ...
                  "\"source_latest_bar_time\":\"" + EASync_ToIso8601(newest_closed) + "\"," +
                  "\"last_error\":\"Expected candle after " + EASync_ToIso8601(last_sent) +
                     "; newest returned " + EASync_ToIso8601(newest_closed) + "\"," +
         ...
         needs_bootstrap[i] = base_bootstrap_needed;
         g_pr_live_sync_pending = true;
         PrintFormat("PriceReporter: exact-time candle pending %s %s checkpoint=%s current=%s newest_closed=%s copied=%d retry=%ds",
                     broker_symbol, timeframe, TimeToString(last_sent),
                     TimeToString(actual_current_bar), TimeToString(newest_closed), copied,
                     retry_seconds);
         if(diagnostics_due)
         {
            if(diagnostic_count > 0) diagnostics_json += ",";
      ...
      // More than the bounded incremental allowance means the terminal was
      // offline long enough to justify a clean snapshot. Market-session gaps
      // do not trigger this because only actual returned bars are counted.
      bool checkpoint_gap = newer_closed_count > PR_FRESHNESS_BARS;
      forced_gap[i] = checkpoint_gap;
      needs_bootstrap[i] = base_bootstrap_needed || checkpoint_gap;
      if(checkpoint_gap) g_pr_bootstrap_pending = true;
   ...
   for(int pass = 0; pass < 2 && ArraySize(selected) < bootstrap_limit; pass++)
   {
      ...
      // starve a manual/background repair indefinitely.
      int selected_before_lane = ArraySize(selected);
      int range_start = (pass == 0) ? 0 : priority_count;
      int range_count = (pass == 0) ? priority_count : candidate_count - priority_count;
      if(range_count <= 0) continue;
      int cursor = (pass == 0) ? g_pr_priority_cursor : g_pr_background_cursor;
      for(int offset = 0; offset < range_count &&
          ArraySize(selected) == selected_before_lane; offset++)
      {
         int index = range_start + ((cursor + offset) % range_count);
         if(!collector_ready[index]) continue;
         string key = candidate_brokers[index] + "|" + candidate_timeframes[index];
         bool generation_due = PriceReporter_CompletedBootstrapGeneration(key) < generations[index];
         if(!needs_bootstrap[index] || (!generation_due && !forced_gap[index])) continue;
         int selected_count = ArraySize(selected);
         ArrayResize(selected, selected_count + 1);
         selected[selected_count] = index;
      }
   }
   // If one lane has no eligible work, use the spare slot elsewhere. Duplicate
       ...
       ArraySize(selected) < bootstrap_limit; offset++)
   {
      ...
      for(int existing = 0; existing < ArraySize(selected); existing++)
         if(selected[existing] == index) already_selected = true;
      if(already_selected || !collector_ready[index]) continue;
      string key = candidate_brokers[index] + "|" + candidate_timeframes[index];
      bool generation_due = PriceReporter_CompletedBootstrapGeneration(key) < generations[index];
      if(!needs_bootstrap[index] || (!generation_due && !forced_gap[index])) continue;
      int selected_count = ArraySize(selected);
   ...
   int bootstrap_series = 0;
   int bootstrap_bars = 0;
   string bootstrap_keys[];
   datetime bootstrap_times[];
   int bootstrap_generations[];
   int bootstrap_counts[];
   for(int selected_index = 0; selected_index < ArraySize(selected); selected_index++)
   {
      ...
      if(copied <= 1)
      {
         PrintFormat("PriceReporter: bootstrap CopyRates unavailable for %s %s, error %d, synchronized=%d; will retry",
                     broker_symbol, timeframe, GetLastError(),
                     (int)SeriesInfoInteger(broker_symbol, period, SERIES_SYNCHRONIZED));
         continue;
      }
      ...
      {
         // The synchronization probe still returned the exact frozen broker
         // cache. Do not upload the same 1,000 rows every minute; leave the
         // series stale and retry MT5 synchronization on the next normal pass.
         PrintFormat("PriceReporter: repair waiting for newer MT5 data %s %s checkpoint=%s",
                     broker_symbol, timeframe, TimeToString(server_checkpoint));
         continue;
      }
      if(bootstrap_series > 0) bootstrap_json += ",";
      bootstrap_json +=
         "{\"broker_symbol\":\"" + EASync_JsonEscape(broker_symbol) + "\","
         "\"timeframe\":\"" + timeframe + "\","
         "\"source_digits\":" + IntegerToString(digits) + ","
         "\"bootstrap_generation\":" + IntegerToString(generations[i]) + ","
         "\"snapshot_complete\":true,"
         "\"bars\":[" + bars_json + "]}";
      bootstrap_series++;
      bootstrap_bars += closed_count;
      int pending = ArraySize(bootstrap_keys);
      ArrayResize(bootstrap_keys, pending + 1);
      ArrayResize(bootstrap_times, pending + 1);
      ArrayResize(bootstrap_generations, pending + 1);
      ArrayResize(bootstrap_counts, pending + 1);
      bootstrap_keys[pending] = broker_symbol + "|" + timeframe;
      bootstrap_times[pending] = newest_time;
      bootstrap_generations[pending] = generations[i];
...
void PriceReporter_Init(const string base_url, const string api_key, const int report_interval_seconds = 60)
{
   ...
   ArrayResize(g_pr_series_keys, 0);
   ArrayResize(g_pr_series_last_sent, 0);
   ArrayResize(g_pr_bootstrap_keys, 0);
...
//============================================================================
// BEGIN inlined module: IndicatorBridge.mqh (v1.0.49)
// Reads server-approved custom-indicator buffers and relays closed-candle
// BUY/SELL events with the terminal's existing private API key. Indicators
// cannot call WebRequest themselves in MT5, so the EA is the network bridge.
//============================================================================
struct IndicatorBridgeSource
{
   string strategy_id;
   string broker_symbol;
...
IndicatorBridgeSource g_ib_sources[];
string g_ib_last_config_response = "";
string g_ib_base_url = "";
string g_ib_api_key = "";

void IndicatorBridge_Clear()
{
   for(int i = 0; i < ArraySize(g_ib_sources); i++)
      if(g_ib_sources[i].handle != INVALID_HANDLE) IndicatorRelease(g_ib_sources[i].handle);
   ArrayResize(g_ib_sources, 0);
}

void IndicatorBridge_Init(const string base_url, const string api_key)
{
   g_ib_base_url = base_url;
   g_ib_api_key = api_key;
   g_ib_last_config_response = "";
   IndicatorBridge_Clear();
}

void IndicatorBridge_Deinit()
{
   IndicatorBridge_Clear();
}

void IndicatorBridge_AddSource(const string strategy_id, const string broker_symbol,
                               const string timeframe_name, const string indicator_name,
                               const int buy_buffer, const int sell_buffer)
{
   if(strategy_id == "" || broker_symbol == "" || indicator_name == "") return;
   ENUM_TIMEFRAMES timeframe = PriceReporter_TimeframeEnum(timeframe_name);
   if(timeframe == PERIOD_CURRENT && timeframe_name != "") return;
   SymbolSelect(broker_symbol, true);
   ResetLastError();
   int handle = iCustom(broker_symbol, timeframe, indicator_name);
   if(handle == INVALID_HANDLE)
   {
      PrintFormat("IndicatorBridge: could not load %s for %s %s, error=%d",
                  indicator_name, broker_symbol, timeframe_name, GetLastError());
      return;
   }
   int index = ArraySize(g_ib_sources);
   ArrayResize(g_ib_sources, index + 1);
   g_ib_sources[index].strategy_id = strategy_id;
   g_ib_sources[index].broker_symbol = broker_symbol;
   g_ib_sources[index].timeframe_name = timeframe_name;
   g_ib_sources[index].indicator_name = indicator_name;
   g_ib_sources[index].buy_buffer = buy_buffer;
   g_ib_sources[index].sell_buffer = sell_buffer;
   g_ib_sources[index].handle = handle;
   g_ib_sources[index].last_closed_bar = 0;
}

void IndicatorBridge_RefreshConfig(const string json)
{
   if(json == "") return;
   int arr_key = StringFind(json, "\"indicator_sources\":[");
   // A transient non-200 ea-sync response has no indicator_sources field.
   // Keep the last known-good handles; a successful response with [] still
   // produces a real config change and clears them intentionally.
   if(arr_key < 0) return;
   int array_open = arr_key + StringLen("\"indicator_sources\":");
   int array_end = StringFind(json, "]", array_open);
   if(array_end < 0) return;
   string config_response = StringSubstr(json, arr_key, array_end - arr_key + 1);
   if(config_response == g_ib_last_config_response) return;
   g_ib_last_config_response = config_response;
   IndicatorBridge_Clear();
   int pos = arr_key + StringLen("\"indicator_sources\":[");
   int json_len = array_end + 1, depth = 0, obj_start = -1;
   while(pos < json_len)
   {
      int ch = StringGetCharacter(json, pos);
      if(ch == '{') { if(depth == 0) obj_start = pos; depth++; }
      else if(ch == '}')
      {
         depth--;
         if(depth == 0 && obj_start >= 0)
         {
            int obj_end = pos + 1;
            IndicatorBridge_AddSource(
               EASync_JsonGetRaw(json, obj_start, obj_end, "strategy_id"),
               EASync_JsonGetRaw(json, obj_start, obj_end, "broker_symbol"),
               EASync_JsonGetRaw(json, obj_start, obj_end, "timeframe"),
               EASync_JsonGetRaw(json, obj_start, obj_end, "indicator_name"),
               (int)EASync_JsonGetInt(json, obj_start, obj_end, "buy_buffer", 0),
               (int)EASync_JsonGetInt(json, obj_start, obj_end, "sell_buffer", 1)
            );
            obj_start = -1;
         }
      }
   ...
   PrintFormat("IndicatorBridge: configured %d external indicator source(s)", ArraySize(g_ib_sources));
}

bool IndicatorBridge_HasValue(const double value)
{
   return MathIsValidNumber(value) && value != EMPTY_VALUE && value != 0.0;
}

void IndicatorBridge_Send(IndicatorBridgeSource &source, const string side, const datetime source_bar_time)
{
   string event_id = source.strategy_id + ":" + source.broker_symbol + ":" +
                     source.timeframe_name + ":" + IntegerToString((long)source_bar_time) + ":" + side;
   string body = "{\"strategy_id\":\"" + EASync_JsonEscape(source.strategy_id) + "\"," +
                 "\"event_id\":\"" + EASync_JsonEscape(event_id) + "\"," +
                 "\"symbol\":\"" + EASync_JsonEscape(source.broker_symbol) + "\"," +
                 "\"timeframe\":\"" + source.timeframe_name + "\"," +
                 "\"side\":\"" + side + "\"," +
                 "\"occurred_at\":\"" + EASync_ToIso8601(TimeGMT()) + "\"," +
                 "\"source_bar_time\":\"" + EASync_ToIso8601(source_bar_time) + "\"}";
   string headers = "Content-Type: application/json\r\nx-api-key: " + g_ib_api_key + "\r\n";
   uchar data[]; uchar result[]; string result_headers;
   int data_len = StringToCharArray(body, data, 0, StringLen(body), CP_UTF8);
   ArrayResize(data, data_len);
   ResetLastError();
   int status = WebRequest("POST", g_ib_base_url + "/functions/v1/external-signal", headers, 2500, data, result, result_headers);
   string response = CharArrayToString(result, 0, WHOLE_ARRAY, CP_UTF8);
   if(status != 202 && status != 200)
      PrintFormat("IndicatorBridge: %s %s %s rejected, HTTP %d error=%d body=%s",
                  source.broker_symbol, source.timeframe_name, side, status, GetLastError(), response);
   else
      PrintFormat("IndicatorBridge: sent %s %s %s, HTTP %d", source.broker_symbol, source.timeframe_name, side, status);
}

void IndicatorBridge_OnTimer()
{
   if(g_ib_base_url == "" || g_ib_api_key == "") return;
   IndicatorBridge_RefreshConfig(EASync_GetLastResponse());
   for(int i = 0; i < ArraySize(g_ib_sources); i++)
   {
      ENUM_TIMEFRAMES timeframe = PriceReporter_TimeframeEnum(g_ib_sources[i].timeframe_name);
      datetime closed_bar = iTime(g_ib_sources[i].broker_symbol, timeframe, 1);
      if(closed_bar <= 0) continue;
      if(g_ib_sources[i].last_closed_bar == 0) { g_ib_sources[i].last_closed_bar = closed_bar; continue; }
      if(closed_bar <= g_ib_sources[i].last_closed_bar) continue;

      double buy_value[1], sell_value[1];
      int buy_copied = CopyBuffer(g_ib_sources[i].handle, g_ib_sources[i].buy_buffer, 1, 1, buy_value);
      int sell_copied = CopyBuffer(g_ib_sources[i].handle, g_ib_sources[i].sell_buffer, 1, 1, sell_value);
      if(buy_copied != 1 || sell_copied != 1)
      {
         PrintFormat("IndicatorBridge: buffers pending for %s %s, buy=%d sell=%d error=%d",
                     g_ib_sources[i].broker_symbol, g_ib_sources[i].timeframe_name,
                     buy_copied, sell_copied, GetLastError());
         continue;
      }
      g_ib_sources[i].last_closed_bar = closed_bar;
      bool buy = buy_copied == 1 && IndicatorBridge_HasValue(buy_value[0]);
      bool sell = sell_copied == 1 && IndicatorBridge_HasValue(sell_value[0]);
      if(buy == sell)
      {
         if(buy) PrintFormat("IndicatorBridge: ambiguous BUY+SELL ignored for %s %s", g_ib_sources[i].broker_symbol, g_ib_sources[i].timeframe_name);
         continue;
      }
      IndicatorBridge_Send(g_ib_sources[i], buy ? "buy" : "sell", closed_bar);
   }
}
//============================================================================
// END inlined module: IndicatorBridge.mqh
//============================================================================
...
int OnInit()
{
   ...
   IndicatorBridge_Init(SupabaseProjectUrl, TerminalApiKey);
...
void OnTimer()
{
   ...
   // A second EA using the same terminal key remains a warm standby, but it
   // must not hammer command, candle, calendar, or symbol endpoints while the
   // authoritative VPS instance owns the lease.
   if(EASync_IsStandby()) return;
