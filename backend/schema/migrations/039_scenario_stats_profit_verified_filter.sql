-- 039_scenario_stats_profit_verified_filter.sql
--
-- Migration 038 gives `positions` (and therefore trade_history, via the
-- ea-sync close paths updated alongside it) a real session/htf_regime for
-- rows previously stuck at null -- including the self-healing reconciler's
-- placeholder rows (profit=0, outcome='breakeven', profit_verified=false)
-- for positions the EA never reported a real close for.
--
-- Those placeholder rows are not real trade outcomes -- see the dashboard's
-- existing profit_verified filtering on every win-rate/P&L view -- so
-- compute_scenario_stats() and its callers must exclude them too, or the
-- adaptive throttle ladder would learn from fabricated breakeven results
-- the moment they picked up real session/htf_regime context. Previously
-- this couldn't happen because those rows never had non-null session/
-- htf_regime, so they were silently excluded by accident; that accidental
-- protection goes away with migration 038, so this closes the gap
